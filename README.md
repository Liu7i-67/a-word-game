...
- **自动战斗**：顶部「自动战斗」按钮一键挂机，模拟真人操作（点首个可战斗对象 → 攻击 → 继续 → 返回游戏循环再战）；体力低于 30% 自动终止（再点会提示体力过低），战败/离开场景/场景无可战斗对象自动停止；点击「战斗ing」随时停止
...
- **装备系统**：状态/物品页、装备详情、使用手持/卸下、等级与负重校验；铁匠出售装备成交后留在列表连续出售（免去「返回铁匠铺→出售装备」往返），同名多件支持一键批量回收
